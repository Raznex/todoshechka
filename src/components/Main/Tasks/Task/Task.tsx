...
const Task: React.FC<ITaskProps> = ({ task, onTaskClick, isActive }) => {
  ...
  return (
    <button type="button" className={ `task__card ${isActive ? 'task__card_active' : ''}` } onClick={ () => onClick(task) }>
      ...
      <p className={ `task__priority ${task.priority === 'LOW' ? 'task__priority_n' : 'task__priority_hidden'}` }>{ task.priority }</p>
      <p className={ `task__priority ${task.priority === 'MEDIUM' ? 'task__priority_s' : 'task__priority_hidden'}` }>{ task.priority }</p>
      <p className={ `task__priority ${task.priority === 'HIGH' ? 'task__priority_v' : 'task__priority_hidden'}` }>{ task.priority }</p>
